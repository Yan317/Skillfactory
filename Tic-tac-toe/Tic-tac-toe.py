...
def checking_for_win_old(play_field_list):
...
def checking_for_win(play_field_list):
    '''Функция проверяет не наступил ли выигрыш.
       ...
       Версия 2.'''
    for line_list in solution_address_set:
        # пробежим по линиям возможного выигрыша
        for cell_row, cell_column in line_list:
            # пробежим по ячейкам линии возможного выигрыша
            value_line = ''
            print(play_field_list(cell_row, cell_column))
            #'value_line += play_field_list(cell_row, cell_column)
        # оцениваем собранные занчения рассматриваемой линии
        if value_line == 'xxx':
            return 'x'
        elif value_line == 'ooo':
            return 'o'
...
def move_user(play_field_list):
    '''Функция обработкит хода пользователя.
       На вход функции подается игровое поле до хода пользователя.
...
def one_game(score_user_comp):
    '''Процедура проведения одной партии.
       ...
       Результат функции счёт по завершении данной партии (с учётом партий совершенных до этой).'''
